...
const openVisitModal = (sponsorId) => {
    ...
    if (sponsor && sponsor.importe) {
        contributionAmount.value = sponsor.importe;
    }
        ...
        content += `
            <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-sm text-gray-600">${sectorVisitedCount}/${stats.total} visitados</span>
                </div>
                <div class="text-sm text-gray-600 mb-2">Recaudado: $${stats.collected.toFixed(2)} de $${stats.expected.toFixed(2)}</div>
                <div class="progress-bar">
                    <div class="progress-bar-fill" style="width: ${Math.min(sectorProgress, 100)}%"></div>
                </div>
                <div class="text-xs text-gray-500 mt-1">${sectorProgress.toFixed(1)}% visitados</div>
            </div>
        `;
    });
...
const filterSponsors = () => {
    ...
    filteredSponsors = sponsors.filter(sponsor => {
        ...
        const matchesStatus = statusValue === 'all' || 
                            ...
                            (statusValue === 'visited' && sponsorStatus === 'visited_no_contribution') ||
...
auth.onAuthStateChanged(async (user) => {
    ...
    if (urlParams.get('mode') === 'qr' && urlParams.get('id')) {
        ...
        database.ref(`users/${userId}/padrinos`).on('value', (snapshot) => {
            ...
            if (currentSearchTerm || currentStatusFilter !== 'all' || currentSectorFilter !== 'all' || currentTagFilter !== 'all') {
                filterSponsors();
            } else {
...
visitModal.addEventListener('change', (e) => {
    if (e.target.name === 'visitStatus') {
        if (e.target.value === 'completed') {
            ...
            if (!contributionAmount.value && currentVisitSponsorId) {
...
visitForm.addEventListener('submit', async (e) => {
    ...
    try {
        if (visitStatus.value === 'reset_pending') {
            ...
            if (confirmed) {
                await database.ref(`users/${userId}/padrinos/${currentVisitSponsorId}/visits`).remove();
                showToast('Padrino marcado como pendiente');
        ...
        showToast('Error al procesar la visita', 'error');
    } finally {
...
sponsorForm.addEventListener('submit', async (e) => {
    ...
    try {
        if (editMode) {
            ...
            await database.ref(`users/${userId}/padrinos`).push(sponsorData);
