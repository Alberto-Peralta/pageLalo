...
waitForFirebase().then(() => {
    ...
    const { ref, onValue, set, update, remove } = window.firebaseServices;
    ...
    function initializeMainApp(authenticatedUserId) {
        ...
        onValue(ref(database, `users/${window.userId}/padrinos`), (snapshot) => {
            const sponsorsData = [];
            if (snapshot.exists()) {
            ...
            if (window.birthdayModule) {
                window.birthdayModule.updateBirthdayCounter(sponsors);
            }
    ...
    const showNewCycleAnimation = () => {
        ...
        setTimeout(() => { newCycleOverlay.classList.remove('show'); }, 1500);
    };
    ...
    const formatPhoneNumber = (phone) => {
        ...
        return cleaned.length === 10 ? `52${cleaned}` : cleaned;
    };

    const makeCall = (phone) => {
        const formatted = formatPhoneNumber(phone);
        if (formatted) window.open(`tel:+${formatted}`, '_self');
    };
        ...
        setTimeout(() => {
            const closeMenu = (e) => {
                if (!actionMenu.contains(e.target)) {
                    if (document.body.contains(actionMenu)) document.body.removeChild(actionMenu);
    ...
    const getVisitStatusInfo = (status) => {
        const statusMap = {
            pending: { icon: 'fas fa-clock', color: 'text-yellow-700', bgColor: 'bg-yellow-100', label: 'Pendiente', description: 'Visita requerida' },
            completed: { icon: 'fas fa-check-circle', color: 'text-green-600', bgColor: 'bg-green-100', label: 'Completado', description: 'Aportó' },
            visited_no_contribution: { icon: 'fas fa-user-clock', color: 'text-blue-600', bgColor: 'bg-blue-100', label: 'Visitado', description: 'No aportó' },
            not_home: { icon: 'fas fa-home', color: 'text-yellow-600', bgColor: 'bg-yellow-100', label: 'No estaba', description: 'No en casa' },
            return_later: { icon: 'fas fa-clock', color: 'text-orange-600', bgColor: 'bg-orange-100', label: 'Regresar', description: 'Pidió que regresemos' }
        };
    ...
    const calculateTotalCollected = () => {
        return sponsors.reduce((total, sponsor) => {
            if (!sponsor.visits || Object.keys(sponsor.visits).length === 0) return total;
            const sortedVisits = Object.values(sponsor.visits).sort((a, b) => b.timestamp - a.timestamp);
            const lastVisit = sortedVisits[0];
    ...
    const calculateTotalExpected = () => sponsors.reduce((total, sponsor) => total + (sponsor.importe || 0), 0);
    const calculateTotalPending = () => Math.max(0, calculateTotalExpected() - calculateTotalCollected());
    const getVisitCount = (sponsor) => sponsor.visits ? Object.keys(sponsor.visits).length : 0;

    const getTotalContributed = (sponsor) => {
        if (getSponsorStatus(sponsor) !== 'completed') return 0;
        if (!sponsor.visits) return 0;
        return Object.values(sponsor.visits).filter(v => v.status === 'completed').reduce((sum, visit) => sum + (visit.amount || 0), 0);
    };
    ...
    const saveVisit = async (sponsorId, visitData) => {
        ...
        const visitRef = ref(database, `users/${window.userId}/padrinos/${sponsorId}/visits/${visitId}`);
        await set(visitRef, { ...visitData, timestamp: Date.now() });
    };
    ...
    const generateHistoryContent = (sponsor) => {
        if (!sponsor.visits || Object.keys(sponsor.visits).length === 0) {
            return `<div class="text-center py-8"><i class="fas fa-history text-gray-400 text-3xl mb-4"></i><p class="text-gray-500">No hay visitas registradas</p></div>`;
        }
        
        const visits = Object.entries(sponsor.visits).map(([id, visit]) => ({ id, ...visit })).sort((a, b) => b.timestamp - a.timestamp);
        
        let content = `<div class="mb-6"><h3 class="text-lg font-bold mb-2">${sponsor.tratamiento || ''} ${sponsor.nombre || 'Padrino'}</h3></div><div class="visit-history">`;
            ...
            content += `
                <div class="visit-entry ${visit.status}">
                    <div class="visit-entry-header">
                        <div class="visit-entry-status ${statusInfo.color}"><i class="${statusInfo.icon} mr-2"></i>${statusInfo.label}</div>
    ...
    const createMainSponsorCard = (sponsor) => {
                ...
                <div class="flex justify-between items-start mb-3">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">${customTag}${statusBadge}</div>
                        </div>
                    </div>
                </div>
        ...
        // Integrar información de cumpleaños
        if (window.birthdayModule) {
            cardHTML = window.birthdayModule.addBirthdayInfoToCard(sponsor, cardHTML);
        }
    ...
    const updateSectorsList = () => {
        sectors.clear();
        sponsors.forEach(sponsor => { if (sponsor.sector) sectors.add(sponsor.sector); });
    ...
    const updateTagsList = () => {
        tags.clear();
        sponsors.forEach(sponsor => { if (sponsor.etiqueta && sponsor.etiqueta.trim()) tags.add(sponsor.etiqueta.trim()); });
    ...
    // Exportar para que birthdayModule pueda acceder
    ...
    const applyFiltersAndRender = () => {
        filteredSponsors = sponsors.filter(sponsor => {
            ...
            if (window.birthdayModule) {
                ...
                if (birthdayFilter) {
                    matchesBirthday = birthdayFilter.value === 'all' || window.birthdayModule.isBirthdayMonth(sponsor);
                }
            }
            
            return matchesSearch && matchesStatus && matchesSector && matchesTag && matchesBirthday;
        });
    ...
    const openModal = (sponsor = null) => {
        ...
        // Asegurar que los campos de cumpleaños existan
        if (window.birthdayModule) {
            window.birthdayModule.enhanceSponsorForm();
        }
        ...
        if (sponsor) {
            ...
            // Cargar fecha de nacimiento y cumpleaños
            const fechaNacimientoInput = document.getElementById('fechaNacimiento');
            if (fechaNacimientoInput) fechaNacimientoInput.value = sponsor.fechaNacimiento || '';
            
            const cumpleanosInput = document.getElementById('cumpleanos');
            if (cumpleanosInput) cumpleanosInput.value = sponsor.cumpleanos || '';
    ...
    sponsorList.addEventListener('click', async (e) => {
        ...
        if (button.matches('.visit-btn')) openVisitModal(id);
        else if (button.matches('.history-btn')) openHistoryModal(id);
        else if (button.matches('.edit-btn')) openModal(sponsors.find(s => s.id === id));
    ...
    confirmDeleteBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            await remove(ref(database, `users/${window.userId}/padrinos/${sponsorToDeleteId}`));
    ...
    visitForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (visitStatus.value === 'reset_pending') {
                ...
                if (confirmed) {
                    await remove(ref(database, `users/${window.userId}/padrinos/${currentVisitSponsorId}/visits`));
    ...
    cancelNewCycleBtn.addEventListener('click', () => newCycleModal.classList.add('hidden'));
    ...
    confirmNewCycleBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            sponsors.forEach(sponsor => {
                if (sponsor.visits) updates[`users/${window.userId}/padrinos/${sponsor.id}/visits`] = null;
            });
    ...
    statusFilter.addEventListener('change', window.handleFilterChange);
    sectorFilter.addEventListener('change', window.handleFilterChange);
    tagFilter.addEventListener('change', window.handleFilterChange);
    ...
    sponsorForm.addEventListener('submit', async (e) => {
        ...
        // Incluir fecha de nacimiento y cumpleaños si existen
        const fechaNacimientoInput = document.getElementById('fechaNacimiento');
        if (fechaNacimientoInput && fechaNacimientoInput.value) {
            sponsorData.fechaNacimiento = fechaNacimientoInput.value;
        }
        
        const cumpleanosInput = document.getElementById('cumpleanos');
        if (cumpleanosInput && cumpleanosInput.value) {
            sponsorData.cumpleanos = cumpleanosInput.value;
        }
            
        try {
            if (editMode) {
                await update(ref(database, `users/${window.userId}/padrinos/${currentSponsorId}`), sponsorData);
                showUpdatedAnimation();
            } else {
                const newKey = Date.now().toString() + Math.random().toString(36).substr(2, 9);
                await set(ref(database, `users/${window.userId}/padrinos/${newKey}`), sponsorData);
    ...
    // Goals/Charts button event listener
    const goalsBtn = document.getElementById('goalsBtn');
    if (goalsBtn) {
        goalsBtn.addEventListener('click', () => {
            if (typeof window.chartsModule !== 'undefined') {
                window.currentSponsorsData = sponsors;
                window.chartsModule.showGoalsChartsModal(sponsors, window.userId);
            }
        });
    }
});